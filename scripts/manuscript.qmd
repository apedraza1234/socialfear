...
Fear is our dependent variable, so the first step was to remove observations with NAs for that variable, as those won't be meaningful for our analysis. 
...
I want to make a note about the interpretation for "mean fear by race". Because our outcome variable is dichotomous (yes or no to having fear of walking alone in neighborhood at night). The mean number approximately gets at the idea of the proportions of a specific racial group that experience fear of walking in neighborhood at night. 


# Tables and Figures

Let's understand our sample a little better. There are 2318 observations and 11 selected variables in our data set. 

```{r}
#| label: get to know data more
view(gss_subset)

skimr::skim(gss_subset)
```
...
gss_subset %>% 
  select(fear, sexnow1, racecombo, degree) %>% 
  gtsummary::tbl_summary()
```


```{r}
#| label: tbl_gender_fear
#| tbl-cap: table showing the distribution of fear by gender
...
```{r}
#| label: tbl_racecombo_fear
#| tbl-cap: table showing the distribution of fear by race
gss_subset %>% 
  tabyl(fear, racecombo) %>% 
  adorn_percentages() %>% 
  adorn_pct_formatting() %>% 
  adorn_ns(position = "front") %>% 
  adorn_title(
...
```{r}
#| label: fig1-fear_gender
#| fig-cap: Bar graph showing the proportion or respondants, by gender, that feared walking alone at night.
#| message: false

gss_subset %>% 
  ggplot(aes(x = sexnow1, y = after_stat(prop), group = fear, 
             fill = fear)) +
  geom_bar(position = "dodge") + 
  scale_y_continuous(labels = scales::percent) + 
  labs(title = "Social Fear by Gender",
