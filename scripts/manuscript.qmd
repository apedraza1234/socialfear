...
**Sociodemographics Variables** 1. "born": Were you born in this country? 2. "age" 3. "sexnow1": Do you describe yourself as male, female, or transgender? *This is self-described gender.* 4. "sex": Code respondent's sex *This is gender presentation as interpreted by the interviewer.* 4. "sexornt": Which of the following best describes you? 5. "racerank1": If you had to choose, which of these races do you identify most with? *Kept this race variable here instead of "race" because it allows more heterogeneity within group.* 6. "hispanic": Are you Spanish, Hispanic, or Latino/Latina? IF YES: Which group are you from? 7. "degree": Respondent's degree. 8. "uscitzn": Are you a US citizen? 9. "dwelown": (Do you/Does your family) own your (home/apartment), pay rent, or what?
...
na_data <- data %>% 
  mutate(across(c(id, age, sex, sexnow1, sexornt, degree, attend, raclive, happy, health, helpful, fair, trust, fear, partnrs5, racerank1, uscitzn, dwelown), ~ifelse(. %in% missing_vec, NA_character_, .)))


#data_complete <- mice(data, m=5)
...
```{r}
#| label: mice imputations to deal with missing data

#data_complete <- mice(no_na_data, m=5)

#summary(complete(data_complete,1))
```
...
tidydata <- data %>% 
  mutate(fear = 
           case_when( fear == ".i:  Inapplicable" | fear == ".n: No answer" | 
                        fear == ".d: Do not Know/Cannot Choose" |fear == ".s: Skipped on Web" ~ NA_character_, 
                      fear == "YES" ~ "yes", 
                      fear == "NO" ~ "no")) %>% 
  filter(fear == "yes" | fear == "no") %>% 
  mutate(racecombo = 
